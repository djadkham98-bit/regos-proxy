...
def build_message(data: dict, selected: set) -> str:
    ...
    store_tops = data.get("store_tops", {})
    grand = 0
    ...
    # Топ-5 по каждому выбранному магазину
    medals = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣"]
    shown_any = False
    for store in ALL_STORES:
        if store not in selected:
            continue
        tops = store_tops.get(store, {})
        top_qty = tops.get("top5_qty", [])
        top_rev = tops.get("top5_rev", [])
        if not top_qty and not top_rev:
            continue
        if not shown_any:
            lines.append("")
            lines.append("📦 *Топ-5 товаров по магазинам*")
            shown_any = True
        lines.append(f"\n🏪 *{store}*")
        if top_qty:
            lines.append("  _По количеству:_")
            for i, item in enumerate(top_qty):
                lines.append(f"  {medals[i]} {item['name']} — {fmt(item['qty'])} шт")
        if top_rev:
            lines.append("  _По выручке:_")
            for i, item in enumerate(top_rev):
                lines.append(f"  {medals[i]} {item['name']} — {fmt(item['rev'])} сум")
